...
// Read-only endpoint — returns the N most recent qa-scan.yml workflow runs.
...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  const limit = Math.min(
    parseInt(event.queryStringParameters?.limit ?? '10', 10) || 10,
    50,
  );

  try {
    const res = await fetch(
      `https://api.github.com/repos/${githubRepository}/actions/workflows/qa-scan.yml/runs?per_page=${limit}`,
    ...
    console.error('qa-list-runs error:', error);
    return {
      statusCode: 500,
      headers: corsHeaders,
      body: JSON.stringify({
        error: 'Failed to fetch QA run list',
