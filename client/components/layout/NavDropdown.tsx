...
export default function NavDropdown({ item }: NavDropdownProps) {
  ...
  return (
    ...
    >
      <Link
        to={item.href}
        className="font-manrope text-[20px] text-white py-[31px] mr-[20px] whitespace-nowrap hover:opacity-80 transition-opacity duration-400 inline-flex items-center gap-1"
      >
        {item.label}
        <ChevronDown
      ...
      >
        {item.children!.map((child, idx) => (
          <Link
            ...
            className="block px-5 py-2.5 font-manrope text-[16px] text-white/90 hover:bg-white/10 hover:text-white transition-colors whitespace-nowrap"
            tabIndex={open ? 0 : -1}
