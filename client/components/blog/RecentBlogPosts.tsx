...
export default function RecentBlogPosts({
  ...
  const seededPosts = (initialPosts || []).slice(0, data.postCount) as PostWithCategory[];
  ...
  useEffect(() => {
    ...
    fetchRecentPosts();
  }, [data.postCount, seededPosts.length]);

  return (
    <section className="bg-white py-[40px] md:py-[60px]">
      <div className="max-w-[2560px] mx-auto w-[95%] md:w-[90%] lg:w-[85%]">
        <div className="text-center mb-[30px] md:mb-[50px]">
          <p className="font-manrope text-[18px] md:text-[24px] leading-tight md:leading-[36px] text-[rgb(107,141,12)] mb-[10px]">
            {data.sectionLabel}
          </p>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="font-manrope text-[18px] text-black/60">
              No posts published yet. Check back soon!
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {posts.slice(0, data.postCount).map((post) => (
